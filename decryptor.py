# 参考原C#代码：
# https://github.com/WXRIW/Lyricify-Lyrics-Helper/blob/c86efa8fd9df10c2f3a88251989df35a44e62f46/Lyricify.Lyrics.Helper/Decrypter/Qrc/DESHelper.cs
# https://github.com/WXRIW/Lyricify-Lyrics-Helper/blob/c86efa8fd9df10c2f3a88251989df35a44e62f46/Lyricify.Lyrics.Helper/Decrypter/Qrc/Decrypter.cs
...
def initial_permutation(state, input_data):
    ...
    state[1] = (bitnum(input_data, 56, 31) | bitnum(input_data, 48, 30) | bitnum(input_data, 40, 29) | bitnum(input_data, 32, 28) |
...
def inverse_permutation(state, input_data):
    ...
    input_data[2] = (bitnum_intr(state[1], 6, 7) | bitnum_intr(state[0], 6, 6) | bitnum_intr(state[1], 14, 5) |
                     bitnum_intr(state[0], 14, 4) | bitnum_intr(state[1], 22, 3) | bitnum_intr(state[0], 22, 2) |
                     bitnum_intr(state[1], 30, 1) | bitnum_intr(state[0], 30, 0))

    input_data[1] = (bitnum_intr(state[1], 5, 7) | bitnum_intr(state[0], 5, 6) | bitnum_intr(state[1], 13, 5) |
                     bitnum_intr(state[0], 13, 4) | bitnum_intr(state[1], 21, 3) | bitnum_intr(state[0], 21, 2) |
                     bitnum_intr(state[1], 29, 1) | bitnum_intr(state[0], 29, 0))

    input_data[0] = (bitnum_intr(state[1], 4, 7) | bitnum_intr(state[0], 4, 6) | bitnum_intr(state[1], 12, 5) |
                     bitnum_intr(state[0], 12, 4) | bitnum_intr(state[1], 20, 3) | bitnum_intr(state[0], 20, 2) |
                     bitnum_intr(state[1], 28, 1) | bitnum_intr(state[0], 28, 0))

    input_data[7] = (bitnum_intr(state[1], 3, 7) | bitnum_intr(state[0], 3, 6) | bitnum_intr(state[1], 11, 5) |
                     bitnum_intr(state[0], 11, 4) | bitnum_intr(state[1], 19, 3) | bitnum_intr(state[0], 19, 2) |
                     bitnum_intr(state[1], 27, 1) | bitnum_intr(state[0], 27, 0))

    input_data[6] = (bitnum_intr(state[1], 2, 7) | bitnum_intr(state[0], 2, 6) | bitnum_intr(state[1], 10, 5) |
                     bitnum_intr(state[0], 10, 4) | bitnum_intr(state[1], 18, 3) | bitnum_intr(state[0], 18, 2) |
                     bitnum_intr(state[1], 26, 1) | bitnum_intr(state[0], 26, 0))

    input_data[5] = (bitnum_intr(state[1], 1, 7) | bitnum_intr(state[0], 1, 6) | bitnum_intr(state[1], 9, 5) |
                     bitnum_intr(state[0], 9, 4) | bitnum_intr(state[1], 17, 3) | bitnum_intr(state[0], 17, 2) |
                     bitnum_intr(state[1], 25, 1) | bitnum_intr(state[0], 25, 0))

    input_data[4] = (bitnum_intr(state[1], 0, 7) | bitnum_intr(state[0], 0, 6) | bitnum_intr(state[1], 8, 5) |
                     bitnum_intr(state[0], 8, 4) | bitnum_intr(state[1], 16, 3) | bitnum_intr(state[0], 16, 2) |
...
def f(state, key):
    ...
    # 位运算
    state = (bitnum_intl(state, 15, 0) | bitnum_intl(state, 6, 1) | bitnum_intl(state, 19, 2) |
...
def crypt(input_data, output, key):
    state = [0, 0]
    idx, temp = 0, 0

    initial_permutation(state, input_data)  # 初始置换

    for idx in range(15):  # 15轮迭代
        temp = state[1]
        state[1] = f(state[1], key[idx]) ^ state[0]
        state[0] = temp
    state[0] = f(state[1], key[15]) ^ state[0]  # 第15轮

    inverse_permutation(state, output)  # 逆置换


def key_schedule(key, schedule, mode):
...
def TripleDESKeySetup(key, schedule, mode):
    if mode == ENCRYPT:
        ...
        key_schedule(key[16:], schedule[2], ENCRYPT)
    else:
        key_schedule(key[0:], schedule[2], DECRYPT)
        key_schedule(key[8:], schedule[1], ENCRYPT)
        key_schedule(key[16:], schedule[0], DECRYPT)


def TripleDESCrypt(input_data, output, key):
    crypt(input_data, output, key[0])
    crypt(output, output, key[1])
    crypt(output, output, key[2])


def QRCDecrypt(encrypted_lyrics: str):
    KEY = bytearray(b"!@#)(*$%123ZXC!@!@#)(NHL")
    encryptedTextByte = bytearray.fromhex(encrypted_lyrics)  # 将文本解析为字节数组

    data = bytearray(len(encryptedTextByte))
    schedule = [[[0] * 6 for _ in range(16)] for _ in range(3)]
    TripleDESKeySetup(KEY, schedule, DECRYPT)

    # 以 8 字节为单位迭代 cryptoTextByte
    for i in range(0, len(encryptedTextByte), 8):
        temp = bytearray(8)
