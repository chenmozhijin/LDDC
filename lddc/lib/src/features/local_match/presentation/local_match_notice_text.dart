...
/// 把本地匹配通知转成可直接渲染的文案。
///
/// SAF 编码 URI 只应存在于数据层：通知 detail 会拼接原生错误与保存结果，
/// 安卓端因此常带 `content://…` 编码路径。这里（通知文案的唯一收口）
/// 统一可读化，SnackBar、内嵌提示等所有出口都不会漏。
String localMatchNoticeText(
  BuildContext context,
  LocalMatchNotice notice, {
  // 安卓目录树显示名：有值时能还原成"内部存储 / Music / …"，
  // 拿不到时退化为文档层级路径（仍不带百分号编码）。
  ...
  return humanizeAndroidSafUris(
    switch (notice.code) {
      ...
      LocalMatchNoticeCode.busySwitchTree =>
        l10n.localMatchNoticeBusySwitchTree,
      ...
      LocalMatchNoticeCode.busyClearQueue =>
        l10n.localMatchNoticeBusyClearQueue,
      ...
      LocalMatchNoticeCode.scanFailed => l10n.localMatchNoticeScanFailed(
        detail,
      ),
      LocalMatchNoticeCode.treeScanFailed =>
        l10n.localMatchNoticeTreeScanFailed(detail),
      LocalMatchNoticeCode.matchCancelled =>
        l10n.localMatchNoticeMatchCancelled,
      LocalMatchNoticeCode.matchCompleted =>
        l10n.localMatchNoticeMatchCompleted(total, success, failed, skipped),
