...
from LDDC.backend.service import DesktopLyricsInstance, LDDCService, api_version, instance_dict, random
...
def test_desktop_lyrics(qtbot: QtBot) -> None:
    ...
    qtbot.wait(20)
    grab(instance.widget, os.path.join(screenshot_path, "desktop_lyrics_transparency"), "widget")
    ...
    qtbot.wait(30)
    grab(instance.widget, os.path.join(screenshot_path, "desktop_lyrics_auto_sreaching"), "widget")

    def check_lyrics() -> bool:
        return bool(instance.lyrics)
    qtbot.waitUntil(check_lyrics, timeout=10000)
    ...
    qtbot.wait(5000)
    grab(instance.widget, os.path.join(screenshot_path, "desktop_lyrics_playing"), "widget")
    ...
    qtbot.wait(200)
    grab(instance.widget.selector, os.path.join(screenshot_path, "desktop_lyrics_selector"))
    instance.widget.selector.close()

    from LDDC.view.local_song_lyrics_db_manager import local_song_lyrics_db_manager
    local_song_lyrics_db_manager.show()
    qtbot.wait(200)
    grab(local_song_lyrics_db_manager, os.path.join(screenshot_path, "desktop_lyrics_db_manager"))
