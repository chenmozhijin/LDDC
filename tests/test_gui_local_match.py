...
def test_gui_local_match(qtbot: QtBot, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    for file_name_mode_index, save2tagmode_index, save_mode_index in product(
        ...
        song_dir = test_artifacts_path / "audio" / "local_match" / f"{file_name_mode.name}_{save2tagmode.name}_{save_mode.name}"
        ...
        save_path = test_artifacts_path / "lyrics"/ "local_match"/ f"{file_name_mode.name}_{save2tagmode.name}_{save_mode.name}"
        ...
        def check_finished() -> bool:
            return main_window.local_match_widget.taskmanager.is_finished() and not main_window.local_match_widget.status_label.text() and bool(main_window.local_match_widget.songs_table.rowCount())

        qtbot.waitUntil(check_finished, timeout=10000)
        assert main_window.local_match_widget.songs_table.rowCount() == len(SONGS_INFO)
        qtbot.wait(20)
        grab(main_window, screenshot_path/  f"local_match_prepare_{file_name_mode.name}_{save2tagmode.name}_{save_mode.name}")
        ...
        qtbot.wait(150)
        grab(main_window, screenshot_path/  f"local_match_finish_{file_name_mode.name}_{save2tagmode.name}_{save_mode.name}")
