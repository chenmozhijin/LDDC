...
from LDDC.ui.desktop_lyrics_control_bar_ui import Ui_DesktopLyricsControlBar
from LDDC.utils.data import cfg
from LDDC.utils.enum import Direction, LyricsFormat, LyricsType, Source
from LDDC.utils.logger import logger
from LDDC.utils.thread import in_main_thread
from LDDC.utils.utils import LimitedSizeDict
        ...
        def file_selected(path: str) -> None:
            try:
                ...
                if lyrics:
                    ...
                    result = {"info": {"source": Source.Local, "path": path}, 'lyrics': lyrics,
        ...
        if self.preview_lyric_result:
            ...
            if isinstance(lyrics, Lyrics):
                if lyrics.types.get("orig") != LyricsType.PlainText:
                    langs = self.get_lyric_langs()
                    self.lyrics_selected.emit(lyrics, self.path,
...
class DesktopLyricsWidget(DesktopLyricsWidgetBase):
    ...
    def set_playing(self, playing: bool) -> None:
        with QMutexLocker(self.playing_mutex):
            ...
            if self.playing:
                in_main_thread(self.control_bar.play_pause_button.setIcon, QIcon(QIcon.fromTheme(QIcon.ThemeIcon.MediaPlaybackPause)))
            else:
                in_main_thread(self.control_bar.play_pause_button.setIcon, QIcon(QIcon.fromTheme(QIcon.ThemeIcon.MediaPlaybackStart)))
