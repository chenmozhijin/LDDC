...
def ass2fsmdata(ass_content: str) -> tuple[dict, FSMultiLyricsData]:
    """解析ASS文件内容为MultiLyricsData"""
    tags = {}
...
def ass2mdata(ass_content: str) -> tuple[dict, MultiLyricsData]:
    fsdata = ass2fsmdata(ass_content)
    return fsdata[0], MultiLyricsData(
        {
            lang: LyricsData(
                [LyricsLine(line.start, line.end, [LyricsWord(word.start, word.end, word.text) for word in line.words]) for line in fsdata[1][lang]],
            )
            for lang in fsdata[1]
        },
    )
